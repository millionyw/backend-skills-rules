...
| 技能 | 说明 |
|------|------|
| [git-add-message](skills/git-add-message/SKILL.md) | **暂存代码与变更记录脱节** — 自动收集会话改动文件、ruff 格式审查与修复 Python 文件、询问暂存、按功能分组生成 commit message 条目并写入变更文档，智能合并到已有相关条目而非简单追加。仅执行 `git add` 和文档写入，禁止执行 `git commit`。 |
