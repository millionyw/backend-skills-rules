...
## Rule 1: Always Load Server Info from `config/servers.json`

**Problem:** Hardcoding IP/password in scripts is fragile and insecure. Server info may change.

**Mandatory approach:**
1. Read `config/servers.json` to get the server's IP, user, password, and distro
2. If the server is not listed, ask the user for connection details and suggest adding it to config
3. If the user mentions a server by name (e.g., "225 服务器"), look up its info in `config/servers.json`
4. Also check the project's `运维/` directory for server-specific READMEs

```python
# Example: load server info from config
import json
...
**Before any server operation**, read the project's `AGENTS_OPS_RULES.md` (path defined in `config/servers.json` → `paths.ops_rules`) for:
...
1. **Identify the server** — look up `config/servers.json` for IP/user/password/distro
2. **Read server-specific notes** — check project's `运维/<server-dir>/README.md` for special config
...
- `references/servers.md` — Human-readable server reference (canonical source: `config/servers.json`)
- `scripts/ssh_run.py` — Ready-to-use base template for new operation scripts
- `config/servers.json` — Machine-readable server connection info (generated by `install.py`)
